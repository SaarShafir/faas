...
The corpus is generated by ffmpeg and transcoded by the real hydrator
transcoder, so what these functions see here is byte-identical to what they see
in the local stack -- canonical mono 16 kHz FLAC, decoded by libsndfile.
...
def hydrated():
    """Corpus file -> canonical FLAC -> decoded samples, through the real
    transcoder. Module-scoped because ffmpeg is the slow part."""
    import io
    ...
    def load(audio_id: str) -> _Handle:
        if audio_id not in cache:
            source = next(CORPUS.glob(f"{audio_id}.*"))
            flac = transcoder.to_canonical_flac(source.read_bytes())
            samples, rate = soundfile.read(io.BytesIO(flac.data), dtype="float32")
...
def test_silence_ratio_is_near_zero_on_continuous_tone(hydrated):
    ...
    payload = _payload(SilenceRatio().process(_ref(), hydrated("tone-440-stereo-44k")))
...
def test_silence_ratio_finds_the_gaps_in_gated_audio(hydrated):
    ...
    payload = _payload(SilenceRatio().process(_ref(), hydrated("speech-like-16k-mono")))
...
def test_a_clean_tone_is_not_reported_as_clipped(hydrated):
    ...
    payload = _payload(ClippingDetect().process(_ref(), hydrated("tone-440-stereo-44k")))
...
def test_zero_crossing_rate_matches_the_tone_frequency(hydrated):
    ...
    payload = _payload(ZeroCrossingRate().process(_ref(), hydrated("tone-440-stereo-44k")))
...
def test_noise_crosses_zero_far_more_often_than_a_tone(hydrated):
    ...
    tone = _payload(function.process(_ref(), hydrated("tone-440-stereo-44k")))
    noise = _payload(function.process(_ref(), hydrated("pink-noise-48k-stereo")))
...
def test_spectral_centroid_lands_on_the_tone(hydrated):
    ...
    payload = _payload(SpectralCentroid().process(_ref(), hydrated("tone-440-stereo-44k")))
...
def test_rolloff_sees_band_limiting_the_sample_rate_no_longer_shows(hydrated):
    """The point of the function: the hydrator resamples the 8 kHz phone file
    to 16 kHz, so its sample rate says wideband and only the spectrum knows
    otherwise."""
    from functions.spectral_rolloff.function import SpectralRolloff

    payload = _payload(SpectralRolloff().process(_ref(), hydrated("phone-8k-narrowband")))
...
def test_snr_is_high_for_gated_audio_and_low_for_steady_noise(hydrated):
    ...
    gated = _payload(function.process(_ref(), hydrated("speech-like-16k-mono")))
    steady = _payload(function.process(_ref(), hydrated("pink-noise-48k-stereo")))
...
def test_vad_finds_speech_in_the_gated_file_and_none_in_silence(hydrated):
    ...
    function = EnergyVad()
    speech = _payload(function.process(_ref(), hydrated("speech-like-16k-mono")))
...
def test_vad_bridges_gaps_rather_than_splitting_every_word(hydrated):
    ...
    payload = _payload(EnergyVad().process(_ref(), hydrated("speech-like-16k-mono")))
...
def test_every_function_returns_a_result_the_sdk_can_stamp(hydrated):
    ...
    audio = hydrated("speech-like-16k-mono")
    for factory in (
        DurationRms,
        SilenceRatio,
        ClippingDetect,
